...
\`\`\`
      ...
      // On JSON parse error the full content is returned
      expect(result.content).toBe(body);
      expect(result.relations).toEqual({});
    });
  });
...
Also mentions commit:abc123 and commit:def456.`;
      ...
      expect(content).toBe(originalContent);
      // During roundtrip, automatically extracted references are preserved
      expect(relations).toEqual(originalRelations);
    });
  });
...
Currently, our project's environment variables are managed via hasyx.config.json with .env generated by the config tool.
...
\`\`\`
      ...
      // Ensure autogenerated keys are not included in the meta block
