...
function resolveVariant(variant: string, config: any) {
  ...
  const optionalConfigs = [
    ...
    'storage', 'pg', 'docker', 'github', 'resend', 'openrouter', 'firebase', 
    'nextAuthSecrets', 'dns', 'cloudflare', 'projectUser', 'vercel', 'githubWebhooks'
  ];
...
function getEnvMappingForType(typeKey: string): Record<string, string> | undefined {
  ...
  if (addSchema && typeof (addSchema as any).meta === 'function') {
    ...
    return itemMeta?.envMapping as Record<string, string> | undefined;
  }
  // Fallback: the schema itself may carry envMapping (non-list)
  return listMeta?.envMapping as Record<string, string> | undefined;
}
...
function generateEnvFile(config: any, variant: string): string {
  ...
  for (const [configType, configData] of Object.entries(resolvedConfig)) {
    ...
    for (const [configKey, envKey] of Object.entries(envMapping)) {
      const value = (configData as any)[configKey];
