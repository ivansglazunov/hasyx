...
export default function InvitesPanel() {
  ...
  async function refresh() {
    ...
    try {
      ...
      // Admin views for global overview (requires admin secret configured)
      try {
        const allI = await hasyx.select<InviteRow[]>({
          table: 'invites',
          where: {},
          returning: ['id', 'code', 'created_at', 'updated_at', 'user_id'],
          role: hasyx.apolloClient._options.secret ? 'admin' : undefined,
        });
  ...
  return (
    <div className="p-4">
      <Card className="mb-4">
        <CardHeader>
          <CardTitle>Invites diagnostics</CardTitle>
