...
// Treat absent env as enabled (backward compatible). Explicit 0/false/no disables.
const isFeatureEnabled = (value?: string) => {
...
export function OAuthButtons() {
  ...
  const enabled = {
    google: isFeatureEnabled(process.env.NEXT_PUBLIC_GOOGLE_AUTH_ENABLED),
    yandex: isFeatureEnabled(process.env.NEXT_PUBLIC_YANDEX_AUTH_ENABLED),
    github: isFeatureEnabled(process.env.NEXT_PUBLIC_GITHUB_AUTH_ENABLED),
    facebook: isFeatureEnabled(process.env.NEXT_PUBLIC_FACEBOOK_AUTH_ENABLED),
    vk: isFeatureEnabled(process.env.NEXT_PUBLIC_VK_AUTH_ENABLED),
  };
      ...
      <Card className="max-w-md mx-auto">
        <CardHeader>
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-lg">{tCommon('loading')}</CardTitle>
          </div>
          <CardDescription>
            {tOAuth('checkingOptions')}
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="flex flex-col space-y-3">
      {enabled.google && (
        <ProviderButton 
          provider="google"
          label={tAuth('continueWith', { provider: 'Google' })}
          icon={
      ...
      {enabled.yandex && (
        <ProviderButton 
          provider="yandex"
          label={tAuth('continueWith', { provider: 'Yandex' })}
          icon={
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {enabled.github && (
        <ProviderButton 
          provider="github"
          label={tAuth('continueWith', { provider: 'GitHub' })}
          icon={
      ...
      {enabled.facebook && (
        <ProviderButton 
          provider="facebook"
          label={tAuth('continueWith', { provider: 'Facebook' })}
          icon={
      ...
      {enabled.vk && (
        <ProviderButton 
          provider="vk"
          label={tAuth('continueWith', { provider: 'VK' })}
          icon={
